...
function buildPaths(opts) {
    ...
    try {
        var www_path = path.join(paths.orig, 'www', 'config.xml');
        var config_path = path.join(paths.orig, 'config.xml');

        if (!fs.existsSync(www_path) && !fs.existsSync(config_path)) {
            throw new Error('could not locate config.xml');
        }
